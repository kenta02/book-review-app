...
function makeApp(setUserId?: number, setUserRole?: string): express.Application {
  const app = express();
  app.use(express.json());
  app.use((req: TestRequest, _res: express.Response, next: express.NextFunction): void => {
    if (setUserId !== undefined) req.userId = setUserId;
    if (setUserRole !== undefined) req.userRole = setUserRole;
    next();
  });
  app.get('/protected', authenticateToken, (req: TestRequest, res: express.Response): void => {
    res.status(200).json({
      success: true,
      userId: req.userId,
...
describe('authenticateToken middleware', () => {
  ...
  it('sets req.userId and calls next when token is valid', async () => {
    ...
    expect(res.body.userRole).toBeUndefined();
  });
});
